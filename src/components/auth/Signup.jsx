...
export default function SignupPage() {
  ...
  return (
    <div className="min-h-screen flex items-start justify-center bg-gradient-to-br from-cream-50 via-amber-50/40 to-yellow-50/30 dark:from-brown-950 dark:via-brown-900 dark:to-brown-950 p-4 pt-8 sm:pt-12">
      <div className="w-full max-w-6xl grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
        ...
        >
          <div className="bg-white/95 dark:bg-brown-800/95 backdrop-blur-sm rounded-2xl border-2 border-gold/30 dark:border-gold/20 shadow-2xl p-6 sm:p-8">
            {/* Header */}
            ...
            <motion.button
              ...
              className="relative w-full flex items-center justify-center gap-3 py-3 rounded-xl text-sm font-medium transition-all duration-300 cursor-pointer group"
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
            >
              {/* Outer Glow - Always visible */}
              <div className="absolute -inset-0.5 rounded-2xl bg-gradient-to-r from-saffron via-gold to-saffron opacity-30 group-hover:opacity-60 blur-sm transition-opacity duration-300" />
              
              {/* Main Button Background - Solid with gradient border */}
              <div className="absolute inset-0 rounded-xl bg-gradient-to-r from-saffron/5 via-gold/5 to-saffron/5 border-2 border-gold/40 dark:border-gold/30 group-hover:border-gold/60 transition-all duration-300" />
            ...
            <div className="relative my-4">
              ...
              <div className="relative flex justify-center">
                <span className="bg-white dark:bg-brown-800 px-4 text-[10px] text-brown-400 dark:text-cream-50/40 uppercase tracking-wider">
            ...
            <form onSubmit={handleSubmit} className="space-y-3.5">
              <AnimatePresence>
                {error && (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: -10 }}
                    className="flex items-start gap-2 rounded-lg border border-divine-red/50 bg-divine-red/10 p-2.5"
                  >
              ...
              <div>
                ...
                <div className="relative">
                  <User className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-brown-400 dark:text-cream-50/30" />
                  <input
                    type="text"
                    value={name}
                    onChange={(e) => setName(e.target.value)}
                    className="w-full rounded-lg border-2 py-2.5 pl-10 pr-4 text-sm outline-none transition-all focus:border-saffron bg-cream-50/50 dark:bg-brown-900/50 border-gold/20 dark:border-gold/10 text-brown-900 dark:text-cream-50 placeholder:text-brown-400 dark:placeholder:text-cream-50/30"
              ...
              <div>
                ...
                <div className="relative">
                  <Mail className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-brown-400 dark:text-cream-50/30" />
                  <input
                    type="email"
                    value={email}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <div className="relative">
                    <Lock className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-brown-400 dark:text-cream-50/30" />
                    <input
                      type={showPassword ? 'text' : 'password'}
                      value={password}
                      onChange={(e) => setPassword(e.target.value)}
                      className="w-full rounded-lg border-2 py-2.5 pl-10 pr-10 text-sm outline-none transition-all focus:border-saffron bg-cream-50/50 dark:bg-brown-900/50 border-gold/20 dark:border-gold/10 text-brown-900 dark:text-cream-50 placeholder:text-brown-400 dark:placeholder:text-cream-50/30"
                    ...
                    <button
                      ...
                      className="absolute right-3 top-1/2 -translate-y-1/2 text-brown-400 hover:text-brown-600 dark:text-cream-50/40 dark:hover:text-cream-50 transition-colors"
                    >
              ...
              {/* Terms - always agreed, cannot be unchecked */}
              <div className="flex items-center gap-2">
                <input
                  ...
                  aria-checked="true"
                  readOnly
                  className="w-4 h-4 rounded border-gold/30 text-saffron focus:ring-saffron/20 cursor-not-allowed accent-saffron"
                />
                <span className="text-xs text-brown-600 dark:text-cream-50/60">
                  I agree to{' '}
              ...
              <motion.button
                type="submit"
                disabled={loading}
                className="relative w-full overflow-hidden rounded-lg bg-gradient-to-r from-saffron to-gold px-4 py-2.5 text-sm font-semibold text-white shadow-lg hover:shadow-xl hover:shadow-saffron/30 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
