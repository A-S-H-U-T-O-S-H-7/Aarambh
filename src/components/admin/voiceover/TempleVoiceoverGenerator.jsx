...
import { generateTempleScript } from '@/lib/utils/generateTempleScript';

export default function TempleVoiceoverGenerator({
  contentId,
  templeData = {},
  ...
  const handleGenerate = async () => {
    if (!contentId) {
      toast.error('Please save the temple first before generating voiceover.');
      return;
    }

    // ─── Generate the script ───
    const { script, language, hasContent } = generateTempleScript(templeData);

    if (!hasContent) {
      toast.error('Not enough content to generate voiceover. Please fill in the temple details.');
      return;
    }

    // Store script preview for display
    setScriptPreview({ script, language });

    setIsGenerating(true);
    setError(null);

    try {
      const result = await generateVoiceover(contentId, 'temples', script, voiceId);
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const result = await deleteVoiceover(contentId, 'temples', voiceoverUrl);
      if (result.success) {
        ...
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-brown-500 dark:text-cream-50/50">
              {voiceoverUrl 
                ? '✅ Temple voiceover is ready' 
                : 'Generate a sacred, storytelling voiceover for this temple'}
            </p>
          </div>
        ...
        {scriptPreview && !voiceoverUrl && (
          <div className={`p-3 rounded-lg border ${
            ...
            <p className="text-xs font-medium text-gold mb-1">
              📝 Script Preview ({scriptPreview.language === 'hi' ? 'हिंदी' : 'English'})
            </p>
