...
export default function StoryVoiceoverGenerator({
  contentId,
  contentText,
  ...
  const handleGenerate = async () => {
    if (!contentId) {
      toast.error('Please save the story first before generating voiceover.');
      return;
    }

    // ─── Combine story-specific fields ───
    ...
    // 4. Moral
    if (cleanMoral) fullText += `. ${cleanMoral}`;
        ...
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-brown-500 dark:text-cream-50/50">
              {voiceoverUrl 
                ? '✅ Story voiceover is ready' 
                : 'Generate a voiceover for this story using ElevenLabs'}
            </p>
          </div>
