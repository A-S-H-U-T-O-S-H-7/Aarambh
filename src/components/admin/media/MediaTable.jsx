// components/admin/media/MediaTable.jsx
'use client';

import Pagination from "../Pagination";
import MediaTableRow from "./MediaTableRow";

export default function MediaTable({ media, currentPage, totalPages, isUpdating, isDark, onEdit, onDelete, onPageChange }) {
  ...
  if (media.length === 0) {
    return (
      <div className={`rounded-2xl border p-12 text-center transition-all duration-300 ${
        ...
        <p className={`text-sm mt-2 ${isDark ? "text-gray-500" : "text-gray-400"}`}>Create your first video or bhajan</p>
      </div>
    );
  }
