// components/admin/festivals/FestivalTableRow.jsx
'use client';

import { Edit, Trash2, Eye, Calendar, Star } from 'lucide-react';
import Link from 'next/link';

export default function FestivalTableRow({ festival, index, currentPage, itemsPerPage, isDark, onEdit, onDelete }) {
  ...
  const featuredImage = festival.featuredImage || (festival.images && festival.images[0]) || null;
    ...
    <tr className={`transition-colors ${
      ...
      <td className="px-6 py-4">
        <div className="w-12 h-12 rounded-lg overflow-hidden bg-gray-200 dark:bg-gray-700">
          {featuredImage ? (
            <img src={featuredImage} alt={festival.title} className="w-full h-full object-cover" />
          ) : (
            <div className="w-full h-full flex items-center justify-center text-2xl">{festival.emoji || '🎊'}</div>
          )}
        </div>
      </td>
      ...
      <td className="px-6 py-4">
        <span className={`inline-flex px-2.5 py-1 text-xs font-medium rounded-full ${getStatusColor(festival.status)}`}>
      ...
      <td className="px-6 py-4">
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => onDelete(festival.id, festival.title, festival.images)}
            className={`p-2 rounded-lg transition-all duration-200 hover:scale-110 active:scale-95 cursor-pointer ${
              isDark
