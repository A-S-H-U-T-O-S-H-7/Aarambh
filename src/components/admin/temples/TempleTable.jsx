// components/admin/temples/TempleTable.jsx
'use client';

import Pagination from '../Pagination';
import TempleTableRow from './TempleTableRow';

export default function TempleTable({ temples, currentPage, totalPages, isUpdating, isDark, onEdit, onDelete, onPageChange }) {
  ...
  if (temples.length === 0) {
    return (
      <div className={`rounded-2xl border p-12 text-center transition-all duration-300 ${
        ...
        <p className={`text-lg ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>No temples found</p>
        <p className={`text-sm mt-2 ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>Create your first temple</p>
      </div>
    );
  }
        ...
        <table className="w-full">
          <thead className={`${
            ...
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider">Title & Location</th>
              <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider">Deity</th>
