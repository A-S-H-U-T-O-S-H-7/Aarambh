// components/admin/stories/StoriesTable.jsx
'use client';

import Pagination from "../Pagination";
import StoriesTableRow from "./StoriesTableRow";

export default function StoriesTable({ stories, currentPage, totalPages, isUpdating, isDark, onEdit, onDelete, onPageChange }) {
  ...
  if (stories.length === 0) {
    return (
      <div className={`rounded-2xl border p-12 text-center transition-all duration-300 ${
        ...
        <p className={`text-lg ${isDark ? "text-gray-400" : "text-gray-500"}`}>No stories found</p>
        <p className={`text-sm mt-2 ${isDark ? "text-gray-500" : "text-gray-400"}`}>Create your first spiritual story</p>
      </div>
    );
  }
