...
// ─── SIDE STORY CARD (CLAYMORPHISM DESIGN) ───
function SideStoryCard({ story, onLike, isLiked, index }) {
  ...
  return (
    <motion.div
      ...
      className="group flex flex-col p-3 rounded-2xl bg-white/70 dark:bg-brown-800/60 backdrop-blur-sm 
                 border border-gold/40 dark:border-gold/15 
                 shadow-[0_4px_20px_rgba(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgba(244,180,0,0.12)] 
    ...
    >
        ...
        <div className="flex-1 min-w-0 flex flex-col">
          ...
          <div>
            ...
            <Link href={`/stories/${story.slug || story.id}`}>
              <h4 className="text-sm font-semibold text-brown-900 dark:text-cream-50 leading-snug group-hover:text-saffron transition-colors line-clamp-2">
                {story.title}
              </h4>
            </Link>
          </div>
          ...
          {/* Description */}
          {story.description && (
            <p className="mt-1 text-[11px] text-brown-500 dark:text-cream-50/60 line-clamp-1">
              {story.description}
            </p>
          )}
        </div>
      ...
      <div className="flex items-center justify-between mt-2 pt-2 border-t border-gold/10 dark:border-gold/10">
        ...
        >
          {isLiked ? (
            <FaHeart className="w-3.5 h-3.5 text-divine-red animate-pulse" />
          ) : (
            <FaRegHeart className="w-3.5 h-3.5 text-brown-300 dark:text-cream-50/30 hover:text-divine-red transition-colors" />
          )}
        </button>
      </div>
    </motion.div>
